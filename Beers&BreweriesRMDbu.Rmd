...
#Beers and Breweries RMD:Exploratory Data Analysis of U.S. Beers and Breweries


## Question Number 1: How many breweries are present in each state?

### This code gets count by state and gives the summary statistics.  The Median breweries per state is 7.00, the Mean is 10.94.  The least amount per state is 1 and the most is 47 (Colorado)
...
# I have determined to leave them out of some of the information (IBU - ALe comparison, median alcohol) where it doesn't effect the result as much and leave in where it does (number of breweries)
...
### The median alcohol is by volume for most states is about 0.056, Utah and New Jersey are atypically low.  
...
### The median IBU by state has more variability that the ABV. 
...
### The State withe the highest ABV is Colorado.  Kentucky and Indiana aren't far behind.  Most breweries have a maximum ABV of 0.100
...
### The states with the most bitter beer are 1st, 2nd, and 3rd, respectively; Oregon, Virginia, and Maine.  
...
### Looking at the graph, The relationship between ABU and IBU does look linear, however there is a node at 0.05 ABV and 20-25 IBU that's dense indicating that most beers are created with these characteristics.  Notice also, that there are some high ABV beers that are not as bitter; This might be beers that don’t have as much hops in them or were created to be less bitter.
...
# Question 8.2; new column for type beer
...
# Question 8.3;  Budweiser would also like to investigate the difference with respect to IBU and ABV between IPAs (India Pale Ales) and other types of Ale (any beer with “Ale” in its name other than IPA).  You decide to use KNN classification to investigate this relationship.  Provide statistical evidence one way or the other.
  ...
  ## This code combines the two previous dataframes by row & filters NA's.  


```{r}

#  Question 8.3; combine by row & !NA
...
```
 ...
 # Question 8.4;  Budweiser would also like to investigate the difference with respect to IBU and ABV between IPAs (India Pale Ales) and other types of Ale (any beer with “Ale” in its name other than IPA).  You decide to use KNN classification to investigate this relationship.  Provide statistical evidence one way or the other.
  ...
  ## This code uses the previously combined dataframe to take 500 tests to check for the best k value (nearest neighbors) to get the best results. 
 ### The best k value for the KNN test is 5, based on the highest point in the graph.

```{r}

# Question 8.4;  Loop for many k and the average of many training / test partition
...
# Question 8.5;  Budweiser would also like to investigate the difference with respect to IBU and ABV between IPAs (India Pale Ales) and other types of Ale (any beer with “Ale” in its name other than IPA).  You decide to use KNN classification to investigate this relationship.  Provide statistical evidence one way or the other.
  ...
  ## Based on the previous test, the best k value for the KNN test is 5 (the highest point in the graph), so we use that value to create a table and a confusion matrix that analyzes the ABV and IBU data to predict the "Ale" beers and the IPAs
### The KNN classification test has an accuracy of 0.8529. with sensitivity 0.8550 and Specificity 0.8505.  This is a very robust analysis and shows that there are consistent differences in alcohol content and IBU between the two beer types.


```{r}


classifications = knn(train[,c(7,8)],test[,c(7,8)],train$type, prob = TRUE, k = 5)
